...
test("fresh launch contract detects an in-place update despite cached Owner modules and latches rejection", { timeout: 10_000 }, async () => {
	...
	await assert.rejects(guard.assertCompatible(), new RegExp(String.raw`protocol_mismatch.*expected ${AGENT_CONTROL_PROTOCOL_VERSION + 1}, received ${AGENT_CONTROL_PROTOCOL_VERSION}`));
	await publish(AGENT_CONTROL_PROTOCOL_VERSION, ChildProcessBootstrapSchema);
...
test("same-version on-disk schema drift is rejected before Pi launch", { timeout: 10_000 }, async () => {
	...
	await assert.rejects(new ChildLaunchContractGuard(pathToFileURL(path)).assertCompatible(), new RegExp(String.raw`schema_drift: expected ${AGENT_CONTROL_PROTOCOL_VERSION}, received ${AGENT_CONTROL_PROTOCOL_VERSION}; missing fields: newRequiredField`));
});
...
test("same-version preflight names incompatible tools constraints without exposing schema values", { timeout: 10_000 }, async () => {
	...
	await assert.rejects(new ChildLaunchContractGuard(pathToFileURL(path)).assertCompatible(), (error: Error) => {
		assert.match(error.message, new RegExp(String.raw`schema_drift: expected ${AGENT_CONTROL_PROTOCOL_VERSION}, received ${AGENT_CONTROL_PROTOCOL_VERSION}; missing fields: none; invalid fields: tools`));
