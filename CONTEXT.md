...
**Run Suspension**:
An evidence-backed exact-Run stop after an unexpected terminal error survives configured native recovery, distinct from Run Failure and a human-issued Interruption Hold. Its reason is either `provider_quota` (exhausted provider quota, carrying the retained quota evidence and any reset time) or `runtime_error` (any other terminal error from a still-usable Runtime, carrying the observed failure stage, error, and provenance). It preserves Request ownership, keeps the exception visible in the Agent status instead of publishing a Runtime Report or starting a Moderator, and blocks implicit continuation until a new human editor Message, an authorized Supervisory Resume Message, or termination; observing its status, selecting it, or changing models does not change the stop. The stop is process-local, so host loss ends it and leaves ordinary dormant work instead of a durable fence.

**Supervisory Resume Message**:
An authorized supervisor's free-form Message requesting that one exact held or suspended Agent Run continue. It uses reserved fixed resumption scheduling, clears only the exact stop against which it was admitted when its standalone Delivery commits, and otherwise remains ordinary Steer direction without gaining power over a later stop.
