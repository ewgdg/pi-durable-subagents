...
export function validateAgentMessageInput(
	...
	if (value.operation === "answer") {
		...
		if (!sameStringList(keys, ["answer", "operation", "requestId"])) {
			throw new CoordinationRecordValidationError("invalid_input: Agent Answer input has an invalid shape");
		}
		if (typeof value.answer !== "string" || value.answer.length === 0) {
			throw new CoordinationRecordValidationError("invalid_input: Agent Answer answer must not be empty");
		}
		if (typeof value.requestId !== "string" || !value.requestId.trim()) throw new CoordinationRecordValidationError("invalid_input: Agent Answer requestId must not be empty");
	...
	if (value.operation === "cancel") {
		...
		if (!sameStringList(keys, ["operation", "reason", "requestMessageId"])) {
			throw new CoordinationRecordValidationError("invalid_input: Request Cancellation input has an invalid shape");
		}
		if (
		...
		if (typeof value.reason !== "string" || value.reason.length === 0) {
			throw new CoordinationRecordValidationError("invalid_input: Request Cancellation reason must not be empty");
		}
		return {
			operation: "cancel",
	...
	if (!sameStringList(keys, ["messageId", "operation"])) {
		throw new CoordinationRecordValidationError("invalid_input: Agent Message poll input has an invalid shape");
	}
	if (value.operation !== "poll" && value.operation !== "retry") {
		throw new CoordinationRecordValidationError("invalid_input: Agent Message operation is unavailable");
	}
	if (typeof value.messageId !== "string" || value.messageId.length === 0) {
		throw new CoordinationRecordValidationError("invalid_input: Agent Message messageId must not be empty");
	}
...
function validateRequestSendInput(value: Record<string, unknown>): RequestSendInput {
	if (!("title" in value)) {
		throw new CoordinationRecordValidationError('invalid_input: Agent Request required field "title" is missing');
	}
	const keys = Object.keys(value).sort();
	const expectedKeys = [
	...
	if (!sameStringList(keys, expectedKeys)) {
		throw new CoordinationRecordValidationError("invalid_input: Agent Request input has an invalid shape");
	}
	if (typeof value.targetAgent !== "string" || value.targetAgent.length === 0) {
		throw new CoordinationRecordValidationError("invalid_input: Agent Request targetAgent must not be empty");
	}
	if (typeof value.question !== "string" || value.question.length === 0) {
		throw new CoordinationRecordValidationError("invalid_input: Agent Request question must not be empty");
	}
	if (typeof value.title !== "string" || !value.title.trim()) {
		throw new CoordinationRecordValidationError("invalid_input: Agent Request title must not be blank");
	}
	if (
		...
		throw new CoordinationRecordValidationError("invalid_input: Agent Request deliveryMode is unavailable");
	}
...
function validateContextPreparation(value: unknown): ContextPreparation {
	if (typeof value !== "object" || value === null || Array.isArray(value)) {
		throw new CoordinationRecordValidationError("invalid_input: Agent Request contextPreparation has an invalid shape");
	}
	...
	if (!CONTINUATION_WORK_SCALES.includes(preparation.workScale as never)) {
		throw new CoordinationRecordValidationError("invalid_input: Agent Request contextPreparation workScale is unavailable");
	}
...
function validateMessageSendInput(
	...
	if (!sameStringList(keys, expectedKeys)) {
		throw new CoordinationRecordValidationError("invalid_input: Agent Message send input has an invalid shape");
	}
	if (typeof value.targetAgent !== "string" || value.targetAgent.length === 0) {
		throw new CoordinationRecordValidationError("invalid_input: Agent Message targetAgent must not be empty");
	}
	if (typeof value.content !== "string" || value.content.length === 0) {
		throw new CoordinationRecordValidationError("invalid_input: Agent Message content must not be empty");
	}
	if (
		value.deliveryMode !== undefined &&
		value.deliveryMode !== "deferred" &&
		...
		throw new CoordinationRecordValidationError("invalid_input: Agent Message deliveryMode is unavailable");
	}
	return {
		operation: "send",
		targetAgent: value.targetAgent,
