...
- Wheel over roster rows, details, headings, spacing, or the scroll indicator moves the selected Agent one entry per physical tick (Pi's native list scrolling, without wrapping); details follow the new selection. Same-direction repeats inside one tick window collapse into a single step, so high-rate terminals still move one entry per tick; reversing direction responds immediately. Clicks and arrow/`j`/`k` keys select as usual; wheel at either bound is inert.
...
- Tabs, Agents/path, Owner footer, borders, and help do not scroll the roster or the mounted editor. Wheel events outside the overlay follow Pi’s underlying UI behavior.
...
The Reports tab hides while empty and uses only its History heading, with the same Owner footer above help. Down from the final item focuses the footer; Up returns to the final item, and Enter opens Owner. Empty tabs focus the footer directly.
