...
- Wheel over roster rows, spacing, or the scroll indicator moves the selected Agent one row per event (Pi's native list scrolling, without wrapping); details follow the new selection. Clicks and arrow/`j`/`k` keys select as usual; wheel at either bound is inert.
