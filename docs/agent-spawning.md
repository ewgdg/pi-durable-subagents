...
When neither `config` nor the captured Template specifies `tools`, a fresh Runtime inherits the current parent's **active tools**, not its full registered tool catalogue or broader capability ceiling. This applies to the Owner and live child parents; current active tools are synchronized before descendant preparation. Dormant ancestry is resolved from its creation rules and current parent inheritance rather than a stale launch configuration. Explicit selections, including an empty array, still replace inheritance.

`tools` and `skills` are startup selections, not capability ceilings. Coordination tools are normalized to the child's role: Owner-only tools such as `workflow_resume` are removed, and role-required tools are always added. Thus `tools: []` selects no optional tools. At startup admission every selected tool must be available in the child's tool registry: a selected tool the child cannot provide fails admission, and the failure names the unavailable tools, the selected tools a loaded extension left inactive, and the child's active tools. Activation is not part of admission. An inherited extension may deactivate selected tools or activate its own during startup (for example by running a converted tool surface such as code mode), because Pi and extensions may equally change active tools after admission, including dynamic loading. Runtime snapshots report the child's active `tools` and its `registeredTools` catalogue; descendants inherit the active tools only, and the catalogue exists to decide availability. Missing selected skills continue to fail preparation.
