...
`config` may override the model and thinking level independently, working directory, withheld tools and skills, explicit system prompt, system-prompt mode, and native loading of trusted project instruction files such as `AGENTS.md` and `CLAUDE.md`. This context-file setting does not inherit the parent conversation. The canonical `config` remains in the parent Spawn call and overlays the captured Template rules whenever a fresh Runtime is prepared. Both `config.model.id` and `config.model.thinking` are optional. A Template model candidate is one atomic (model, thinking) pair, because a thinking level describes the capabilities of the model it was paired with. Omitting `config.model` or providing `{}` uses the first available Template candidate, or the current parent Runtime when the Template has no model candidates. Omitting only `config.model.id` uses a candidate model with an explicit `config.model.thinking`. Supplying `config.model.id` discards the candidate pair's thinking level and uses the current parent Runtime thinking level unless `config.model.thinking` is given, and it needs no available Template candidate. Explicit `inherit` always uses that field from the current parent Runtime, even with a Template. Template candidates are resolved only while `config.model.id` is omitted; if candidates are configured but none are available, resolution fails. `excludeTools` and `excludeSkills` add to the captured Template rules, deduplicated in that order; a Spawn restriction cannot lift a Template exclusion. Child extension selection is `inherit` or `none`; arbitrary per-child extension paths are not accepted.
