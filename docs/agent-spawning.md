...
`config` may override the model and thinking level, working directory, withheld tools and skills, explicit system prompt, system-prompt mode, and native loading of trusted project instruction files such as `AGENTS.md` and `CLAUDE.md`. This context-file setting does not inherit the parent conversation. The canonical `config` remains in the parent Spawn call and overlays the captured Template rules whenever a fresh Runtime is prepared. A model selection is one atomic (model, thinking) pair, because a thinking level describes the capabilities of the model it was paired with: `config.model`, when supplied, requires both `id` and `thinking`, and the tool and control schemas reject a half-pair. A half-pair would also be unstable, since candidate availability is resolved again for every Runtime and the level could otherwise land on a different model than the caller chose. Omitting `config.model` uses the first available Template candidate pair, or the current parent Runtime pair when the Template has no model candidates; if candidates are configured but none are available, resolution fails. Explicit `inherit` selects one field from the current parent Runtime, so `{ id: "inherit", thinking: "inherit" }` states full inheritance. `excludeTools` and `excludeSkills` add to the captured Template rules, deduplicated in that order; a Spawn restriction cannot lift a Template exclusion. Child extension selection is `inherit` or `none`; arbitrary per-child extension paths are not accepted. A Spawn call recorded before the pair rule keeps resolving: a recorded selection that names only the model takes its thinking level from the current parent Runtime, and one that names only the thinking level takes the first available Template candidate model, else the parent model.

Thinking levels follow the public `ThinkingLevel` type from `@earendil-works/pi-agent-core`. Coordination maintains one runtime list, compile-time checked for invalid and missing Pi levels, and derives its validators and schemas from that list. The Spawn-only `inherit` sentinel is separate; the Spawn schema is not restricted to a model’s supported levels. Runtime Preparation resolves an unsupported level through Pi’s own capability clamp — the nearest supported level, searching upward before downward, and `off` for a model without reasoning support — so the launch specification, the recorded configuration, and the level the child really runs agree.
...
A fresh Runtime's tools and skills belong to that Runtime, never to its parent. It starts from its own default surface — Pi's defaults plus whatever its own extensions activate — and discovers skills for its own working directory and agent directory. The parent's active tools, registered tool catalogue, and loaded skills are not inheritance inputs, for the Owner or for a live child parent; dormant ancestry is resolved from its creation rules rather than a stale launch configuration.

`excludeTools` and `excludeSkills` are withhold filters: they remove names from the child's own baseline surface and from the skills it discovers. They grant nothing, and a name that is absent changes nothing, so a filter never fails a Run. Startup admission makes no claim about which tools a child has. Exclusions accumulate from the captured Template and the canonical Spawn configuration, and the bridge applies them when startup completes, after inherited extensions finished rewiring the surface and before any input is admitted. Participation is the child's own guarantee: the bridge keeps its role coordination tools active (`agent_message`, `agent_wait`, `agent_observe`, `agent_control`, `agent_spawn` or `moderator_control` and `report_to_user`, and `ask_user`), so neither a filter nor an extension leaves a child mute. Pi and extensions may change active tools afterwards, including dynamic loading; a deny list is scope control, not a boundary. Runtime snapshots report the child's active `tools` and loaded `skills`, while the launch configuration records the exclusions. Skill discovery failures such as a non-absolute source path still fail preparation.
...
Repeating `agent_spawn` creates a sibling. Unfiltered direct-child search returns children in canonical spawn-call order; filtered results use relevance first and that order as their deterministic tie-breaker. Observation is passive and supports status lookup by full ID, unique ID suffix, or authorized exact label, plus bounded composable search over authorized Agent scopes. It returns identity, structural relationship, and live Run state without exposing a Pi session or Run handle; search never prepares a dormant Runtime. See [Run supervision](run-supervision.md) for selector precedence and observation authority.
