...
When neither `config` nor the captured Template specifies `allowedTools`, a fresh Runtime inherits the current parent's **active tools**, not its full registered tool catalogue or broader capability ceiling. This applies to the Owner and live child parents; current active tools are synchronized before descendant preparation. Dormant ancestry is resolved from its creation rules and current parent inheritance rather than a stale launch configuration. Explicit selections, including an empty array, still replace inheritance.

The resolved `allowedTools` is a capability ceiling for the child, not an exact active-tool list. Pi and the selected extensions decide which allowed tools are registered and active, and may change their order during the Runtime. An explicitly allowed tool may therefore be unavailable or inactive. Role-required coordination tools are always added to the allowlist. Runtime startup rejects only an active tool outside the resolved allowlist.
