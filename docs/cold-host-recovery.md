...
A quota stop is process-local and does not survive host loss. An Agent whose Run stopped on exhausted quota recovers dormant like any other interrupted work: its Requests, Answer Obligations, and last provider error remain in its transcripts, while the stop itself, the exact stopped Run, and any captured native input are gone. Quota-specific suppression of reminders and moderation ends with it, so the recovered Agent is ordinary unfinished work until it is attempted again. Explicit recovery may therefore re-admit the Agent's captured undelivered work and start a successor Run; while quota is still exhausted, that attempt re-suspends on the same evidence without producing model output. Selecting the recovered Agent prepares an ordinary editor without model generation. See [quota suspension](run-supervision.md#quota-suspension).
