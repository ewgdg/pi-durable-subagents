...
The tool schema states `deliveryMode` and its delivery rules in both authoring operations rather than sharing them through `$defs` and `$ref`. Tool declarations stay free of resolution keywords as a project policy, because each transport resolves them differently: Gemini cannot resolve a reference inside a schema that declares `$id`, some transports strip definitions while keeping references, and strict constrained sampling rejects references outright. Inlining also matches how Gemini already expands references per use site. Calls remain flat, and other operations do not accept that field. Delivery rules belong in the parameter description; cross-operation Answer obligations remain in tool guidance.
