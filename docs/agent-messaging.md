...
One spacing rule holds across delivered and sent coordination blocks, collapsed or expanded: a badge hugs the body it introduces, and a blank row always means a new item. Blank rows therefore appear only between joined entities — batched Delivery items, Wait Answers, and the spawn summary versus the Creation Request block it created — never between a header and its own payload. Expanding changes the body renderer only, so it adds body rows instead of reflowing the block; selecting the badge as the item boundary keeps that boundary visible inside joined blocks, where a header-to-body gap would be indistinguishable from an item-to-item gap.
