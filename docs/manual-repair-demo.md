...
3. Run `/agents` to see the real Moderator record: pre-commit the menu shows only the Moderator with no Owner row at all. Switching joins and releases Runtimes without cross-host adoption, drafts preserved, no auto-resume.
...
7. `moderator_control resolve` sends handling to Dormant, history retained. Repaired Owner reopens idle until a new human message. `repair_commit` auto-admits under the same trigger authority: genuine fresh admission from the repaired transcript on disk (join/release without adoption, fresh read, drafts preserved, idle hold, no auto-resume, no view switch, Moderator undisturbed). The repaired Owner reopens live-idle with human-only hold and zero turns before a new human message. While admission is in flight or has failed, `/agents` opens with the Moderator plus a greyed non-selectable pending row (verified identity plus repaired transcript path, never a fake live record, never clickable); post-admission it opens with the normal live Owner. User returns via `/agents`. Failed admission keeps committed data plus journal intact, shows a truthful admission error in place, and stays in repair context with diagnostics available. Never rollback, never strand without menu. The menu opens in every repair state and never strands.
...
- The repair Moderator's /agents switcher shows only itself pre-commit (no Owner row at all), itself plus a greyed non-selectable pending row while admission is in flight or has failed (verified identity plus transcript path plus stage, never a fake live record, never clickable), and itself plus the normal live Owner post-admission. Never other agents. Spawn, message,
