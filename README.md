...
- **Messaging and Requests:** `agent_message` supports immutable titled Requests, explicitly targeted Answers, retrieval, and cancellation. Deferred Requests enter in admission order when the recipient waits or settles; Steer retains priority. Agents choose their Answer order. `agent_wait` joins all outstanding outbound Request Answers or selected IDs/unique suffixes without consuming child execution capacity while parked.
- **Visible obligations:** `agent_observe` lists your outstanding incoming Requests by ID, requester, and title, or retrieves the exact full Request by ID/unique suffix. Answer receipts identify the originating Request by title and ID.
